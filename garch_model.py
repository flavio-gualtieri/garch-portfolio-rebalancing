...
class PortfolioRebalancing:
    ...
    def compute_strategy(self, w_0, h_0, z_t):
        ...
        for t in range(self.T - 2, -1, -1):
            self.pi_t[t] = (
                ...
                - (self.theta + (self.lmbda + 0.5)) * 2 * self.alpha * self.E_t[t + 1] / (1 - 2 * self.alpha * self.E_t[t + 1]-self.gamma)
            )
        ...
        for t in range(1, self.T):
            w_t[t] = (
                ...
                + self.pi_t[t - 1] * np.sqrt(self.h_t) * z_t + np.log(1 + 0.01)
            )
    ...
    def compute_strategy_with_rebalancing(self, v_0, h_0, z_t, r_f):
        ...
        for t in range(1, self.T):
            ...
            # Total wealth after returns
            v_t[t] = risky_holdings + risk_free_holdings

            # Rebalance to achieve new target weights
